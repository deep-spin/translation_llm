...
import translation_llm.model as llama
...
def freeze_non_lora_weights(model: nn.Module, bias: str = 'none') -> None:
...
def state_dict(model: nn.Module, bias: str = 'none') -> Dict[str, torch.Tensor]:
...
@contextmanager
def apply(r, alpha, dropout, enabled: bool = True):
